...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        operator, operands = expr.first, expr.rest

        op_eval = scheme_eval(operator, env)
        operand_eval = operands.map(lambda x: scheme_eval(x, env))

        return scheme_apply(op_eval, operand_eval, env)        
        # END PROBLEM 3
